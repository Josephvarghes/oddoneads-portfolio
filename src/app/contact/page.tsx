...
function ContactPlannerForm() {
  ...
  useEffect(() => {
    if (vibe) {
      ...
      let message = "";
      if (vibe === "ethereal-dreamer") {
        eventType = "pre-wedding";
        message = "Hi! We'd love to check availability for the Ethereal Dreamer (Airy & Whimsical) vibe.";
      } else if (vibe === "cinematic-luxury") {
        eventType = "destination-weddings";
        message = "Hi! We'd love to consult about a luxury Cinematic Wedding Film package for our destination wedding.";
      } else if (vibe === "raw-storyteller") {
        eventType = "wedding-films";
        message = "Hi! We love the Raw Storyteller aesthetic. We are looking for candid moments and motion pictures.";
      } else if (vibe === "modern-editorial") {
        eventType = "corporate-events";
        message = "Hi! We'd like to check details for premium Modern Editorial/Corporate Event coverage.";
      }
      setFormState((prev) => ({ ...prev, eventType, message }));
    }
  }, [vibe]);
  ...
  return (
    <div className="lg:col-span-7 bg-charcoal-900 border border-white/[0.04] p-8 md:p-12 relative overflow-hidden rounded-2xl">
      {/* Header / Sub */}
      <div className="mb-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <button
          ...
          className="w-full py-4 bg-brand-gradient hover:opacity-90 text-charcoal-950 font-bold uppercase tracking-widest text-xs flex items-center justify-center gap-2 transition-all duration-300 shadow-lg shadow-brand-purple/20 cursor-pointer disabled:opacity-50"
        >
          {isSubmitting ? (
      ...
      <AnimatePresence>
        {showSuccess && (
          ...
          >
            ...
            >
              <div className="w-16 h-16 bg-brand-pink/10 border border-brand-pink/30 rounded-full flex items-center justify-center text-brand-pink mb-6 animate-pulse">
                <CheckCircle2 size={32} />
              </div>
              
              <span className="text-[10px] tracking-[0.4em] text-brand-pink uppercase font-semibold block mb-2">
                Request Confirmed
              </span>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-start">
          ...
          <div className="lg:col-span-5 space-y-8">
            <div>
              <span className="text-[10px] uppercase tracking-widest text-brand-purple font-semibold mb-2 block">
                Direct Inquiries
              </span>
            ...
            <div className="space-y-4 text-xs font-light">
              {/* Phone Prominent */}
              <div className="bg-charcoal-900 border border-white/[0.04] p-6 flex items-start gap-4 rounded-xl">
                <div className="w-10 h-10 bg-brand-teal/5 border border-brand-teal/15 flex items-center justify-center text-brand-teal flex-shrink-0">
                  <Phone size={16} />
                </div>
                <div>
                  <h4 className="font-serif text-white font-medium text-sm mb-1">Direct Hotline</h4>
                  <a href="tel:+919497381830" className="text-brand-teal hover:text-brand-pink transition-colors text-base font-semibold block mt-1 tracking-wider">
                    +91 9497381830
                  </a>
                </div>
              </div>
              ...
              {/* Locations Grid */}
              <div className="bg-charcoal-900 border border-white/[0.04] p-6 flex items-start gap-4 rounded-xl">
                <div className="w-10 h-10 bg-brand-purple/5 border border-brand-purple/15 flex items-center justify-center text-brand-purple flex-shrink-0">
                  <MapPin size={16} />
                </div>
                <div>
          ...
          {/* RIGHT COLUMN: BOOKING FORM WITH SUSPENSE */}
          <Suspense fallback={
            <div className="lg:col-span-7 bg-charcoal-900 border border-white/[0.04] p-8 md:p-12 text-center text-xs text-charcoal-400 rounded-2xl">
              Loading Luxury Planner...
            </div>
          }>
            <ContactPlannerForm />
          </Suspense>
